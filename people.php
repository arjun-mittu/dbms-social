...
<div class="w3-main" style="margin-left:250px">
  ...
  <header class="w3-container w3-xlarge">
	<h1><p class="w3-left" style = "margin: 10px">PEOPLE</p></h1>
  ...
  <?php 
	...
	$query = "select * from signup where user_name not in ('".$_SESSION['username']."')";
	$query = mysqli_query($conn,$query);
    $i=0;
    $k=0;
	while($row = mysqli_fetch_array($query)){
		...
		echo "<img src = '".$row['photo']."' width = 125 height = 100>";
		echo "<div class='desc'>";
		?>
		<form action= "<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method='post'>
		<input type='hidden' name = 'id' value = "<?php echo $row['id']; ?>">
		<?php
		echo "<input type='submit' style = 'margin-top:5px' name = 'profile' class='btn btn-primary' value = 'Friend Him'>";
		echo '</form>';
		...
		echo "<img src = '".$row['photo']."' width = 125 height = 100>";
		echo "<div class='desc'>";
		echo "<input type='submit' style = 'margin-top:5px' name = 'profile' class='btn btn-primary' style = 'margin-top:20px' value = 'Friend Him'>";
		echo '</div>';
		echo '</div>';
            echo '</div>';
            $i=$i+1;
	...
	if(isset($_POST['profile'])){
			$id = $_POST['id'];
			$query1 = "insert into friend values (".$row0['id'].",".$id.")";
